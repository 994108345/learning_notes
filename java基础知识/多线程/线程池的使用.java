线程池的使用
线程池的种类：
	1.CachedThreadPool:根据程序的运行需要去创建线程，它会为每一个任务添加一个新的线程，这边有一个超时机制，当空闲的线程超过60s内没有用到的话，就会被回收。缺点就是没有考虑到系统的实际内存大小。
	2.SingleThreadExecutor:就像线程数为1的FixedThreadPool,单线程去跑任务，不处理并发的操作，不会被回收。
	3.FixedThreadPool:创建可容纳固定数量线程的池子，响应的速度快，适合长期任务。
	4.ScheduledThreadPool:创建一个固定大小的线程池，唯一一个有延迟执行和周期重复执行的线程池。它的核心线程池固定，非核心线程的数量没有限制，但是闲置时会立即会被回收。
创建线程池：
