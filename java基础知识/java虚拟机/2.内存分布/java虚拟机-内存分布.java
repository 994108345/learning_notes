...
2.2.1 程序计数器
	...
	如果政治执行的是Native方法，这个计数器则为空。网上看了一下：原因是因为此时会再开一个线程去执行native方法，新线程的程序计数器是null，旧线程的程序计数器还是自己原来的那个计数器，且旧线程处于阻塞状态，等新线程执行完毕。
	这个内存区是唯一在Java虚拟机规范中没有规定任何OutOfMemoryError（内存溢出）情况的区域。
2.2.2 Java虚拟机栈
